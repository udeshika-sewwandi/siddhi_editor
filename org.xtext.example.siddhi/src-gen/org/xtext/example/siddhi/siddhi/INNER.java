/**
 * generated by Xtext 2.11.0
 */
package org.xtext.example.siddhi.siddhi;

import org.eclipse.emf.ecore.EObject;

/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>INNER</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * </p>
 * <ul>
 *   <li>{@link org.xtext.example.siddhi.siddhi.INNER#getInner <em>Inner</em>}</li>
 * </ul>
 *
 * @see org.xtext.example.siddhi.siddhi.SiddhiPackage#getINNER()
 * @model
 * @generated
 */
public interface INNER extends EObject
{
  /**
   * Returns the value of the '<em><b>Inner</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>Inner</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>Inner</em>' attribute.
   * @see #setInner(String)
   * @see org.xtext.example.siddhi.siddhi.SiddhiPackage#getINNER_Inner()
   * @model
   * @generated
   */
  String getInner();

  /**
   * Sets the value of the '{@link org.xtext.example.siddhi.siddhi.INNER#getInner <em>Inner</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>Inner</em>' attribute.
   * @see #getInner()
   * @generated
   */
  void setInner(String value);

} // INNER
